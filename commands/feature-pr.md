...
- 본 명령은 PR 만 생성한다. develop 머지는 PR 머지로 처리하고, 브랜치 정리는 `/feature-finish` 가 담당한다.
...
```bash
PRE_PR_SHA=$(git rev-parse HEAD)
```

이후 단계에서 문제가 생기면 `git reset --hard "$PRE_PR_SHA"` 로 되돌릴 수 있도록 사용자에게 출력.
...
- **결과가 존재함 (원격에 이미 있음)** → squash rebase 후의 push 는 강제 푸시가 필요함. **자동으로 진행하지 않는다.** 다음 메시지를 출력하고 중단:
  ...
  ⚠ 원격에 feature/<id> 가 이미 존재합니다.
    ...
    선택지:
      ...
      c) 중단하고 PRE_PR_SHA(<sha>) 로 reset --hard 후 재구성
...
✓ feature/<id> PR 생성 완료
  ...
  PR: <url>
  rollback: git reset --hard <PRE_PR_SHA>
  next: PR 머지 후 /feature-finish 로 브랜치 정리
```
