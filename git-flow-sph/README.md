...
```bash
# 번들 풀기
unzip git-flow-sph.plugin -d ~/.claude/plugins/external_plugins/

# Claude Code 재시작 또는 /plugin reload
```

`~/.claude/plugins/external_plugins/git-flow-sph/.claude-plugin/plugin.json` 이 보이면 정상입니다.

### 2) 마켓플레이스 형태로 설치 (선택)

자체 호스팅 마켓플레이스에서 가져오는 경우, 마켓플레이스의 `plugins/` 하위에 디렉터리를 그대로 두면 됩니다.
